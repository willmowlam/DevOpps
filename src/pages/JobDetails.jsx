      ...
      <div className="w-full p-6 bg-white border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700">
        <div className='flex flex-row'>
          <div>
            ...
            <p><i className="fa-regular fa-building text-indigo-500"></i> <strong>Company:</strong> {data.companyName}</p>
            <div className="my-4 font-normal text-gray-700 dark:text-gray-400">{parse(data.descriptionHtml)}</div>
          </div>
        ...
        <div className='flex flex-row sm:flex-col md:flex-row lg:flex-row justify-evenly flex-wrap gap-4 py-3 border-t-1 border-b-1'>
          ...
          <p><i className="fa-regular fa-calendar text-indigo-500"></i> <strong>Date posted:</strong> {data.postedTimeAgo}</p>
        </div>
        {data.applyUrl && (
        <div className='flex flex-row sm:flex-col md:flex-row lg:flex-row justify-evenly flex-wrap gap-4 py-3 border-b-1'>
          <h3 className="font-semibold">Apply here</h3>
            <p><a href={data.applyUrl} target='_blank' className='text-sm hover:text-indigo-400'>APPLY <i className="fa-regular fa-share-from-square text-indigo-500"></i></a></p>
        </div>
        )}
      </div>
    </>
